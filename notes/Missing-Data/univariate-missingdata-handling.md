...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	* For both categirical and numerical

	1] Random 
		-> adding random value from th evalues presentin the rows
		-> easy to implement
		-> works for both categirical and numerical
		-> preserves the variance and distribution of data 
		-> applied for linear algo , doesnt work that good with tree based algo
		-> memory heavy for deployment because the random value is taken from the training data

	2] Missing indicator
		-> adding a new column for the column missing values 
		-> not always usable but you shud try it 
		-> the model learns to differentiate between missing values and non missing values

    3] Autoatically selecting the value for imputation
       GridCV class
